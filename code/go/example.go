...
func main() {
	s, err := runQuery("bolt://demo.neo4jlabs.com:7687", "northwind", "northwind")
	if err != nil {
		panic(err)
	}
	fmt.Println(s)
...
func runQuery(uri, username, password string) ([]string, error) {
	...
	defer driver.Close()
	sessionConfig := neo4j.SessionConfig{AccessMode: neo4j.AccessModeRead, DatabaseName: "northwind"}
